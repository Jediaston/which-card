...
- **Watchlist Ledger** — a stock watchlist with analyst notes, saved locally via `localStorage`. Prices/analysis are compiled by hand in a Cursor chat (there's no reliable key-free stock API a static page can call), so ask in chat to refresh a ticker rather than expecting live data. Live at [https://jediaston.github.io/which-card/watchlist-ledger/](https://jediaston.github.io/which-card/watchlist-ledger/).
