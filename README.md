...
- **Medora** — a community-reviewed directory of hospitals, clinics, veterinarians, care homes, and private practices across Bacolod, Silay, and Talisay, with filtering by type/city, search, ratings, and reviews (saved locally via `localStorage`, with a simple built-in check for spammy/bot-like review patterns). Fully self-contained, no network calls beyond loading fonts/icons. Live at [https://jediaston.github.io/which-card/medora/](https://jediaston.github.io/which-card/medora/).
