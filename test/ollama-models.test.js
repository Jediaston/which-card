...
describe("chrome tokens + earthy people strip", () => {
  ...
  it("shows full standing people on the empty state only", () => {
    const html = readFileSync(new URL("../ollama-chat/index.html", import.meta.url), "utf8");
    ...
    assert.match(text, /full figures/i);
    assert.doesNotMatch(text, /waist down/i);
    assert.match(text, /viewBox="10 0 360 490"/);
